...
function PlantDetail() {
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="text-center">
          ...
          <Link
            to="/"
            className="text-green-700 font-semibold hover:underline focus:outline-none focus:ring-2 focus:ring-green-500 rounded"
          >
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <Link
        to="/"
        className="text-green-700 font-semibold hover:underline mb-6 inline-block focus:outline-none focus:ring-2 focus:ring-green-500 rounded"
      >
        &larr; Back to all plants
      </Link>
      <img
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <button
          ...
          className="flex-shrink-0 text-3xl leading-none mt-1 focus:outline-none focus:ring-2 focus:ring-green-500 rounded"
        >
          {favourited ? '★' : '☆'}
        </button>
      </div>
      ...
      {plant.uses && plant.uses.length > 0 ? (
        <div>
          ...
          <ul className="flex flex-wrap gap-2" aria-label="Plant uses">
            {plant.uses.map((use) => (
              <li
                key={use}
